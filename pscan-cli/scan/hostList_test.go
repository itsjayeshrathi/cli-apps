	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if hl.Hosts[1] != tc.host {
				t.Errorf("Expected host name %q as index 1, got %q instead", tc.host, hl.Hosts[1])
			}
		})
	}
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr != nil {
				...
				if !errors.Is(err, tc.expectErr) {
					t.Errorf("Expected %q, got instead %q", tc.expectErr, err)
				}
				return
			}
...
func TestSaveLoad(t *testing.T) {
	...
	if err := hl1.Save(tf.Name()); err != nil {
		t.Fatalf("Error saving list to file file: %s", err)
	}
